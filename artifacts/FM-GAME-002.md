...
| --- | --- |
| Hub-ready | **Yes** |
| Origin SHA | `0e840e924e990385c2905c2b88200d1e4a1f95f6` |
| GitHub SHA | `94a9c0b49ebbb43d3e1509c8c83871fd34d03efb` |
...
Pass 64 trees match on the three edited paths (byte-identical). Pass 56–63 kept (BRACE SHRUG, SPLICE RELEASE, HOLD YANK, DRY→BACK, FIRST LINE, WEST LIFT, HOLD / FALL, DUSK CLOSE). HOME n stays aborted. Hub counter family frozen. Cold clone still has `Assets/`, `Packages/`, `ProjectSettings/`, USB, `playable/`, artifacts tape. Import workflows self-delete.
...
- **Colony Haul → Play Vertical Slice** — you take the watch (Farm first). South Farm blooms **FARM**, Hub click drops **RAIL**, first cart pops **HAUL**. Press **U** for **WEST LIFT**. West Splash **SLAMS**. Last raids **DUSK CLOSE** onto the Hub. Survive six waves and the mesa *holds*.
...
- GitHub SHA ≠ Origin SHA (parallel history); pass-64 file bytes match.
- Course-correct: no new Hub counter chips. Named leftovers plus SPLASH SLAM shipped. Freeze still open until ~16:30 for one more feel beat if crisp (barrier plant / CREW UP arrival / kill punch).
