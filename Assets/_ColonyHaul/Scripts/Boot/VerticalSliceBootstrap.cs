    ...
    public sealed class VerticalSliceBootstrap : MonoBehaviour
    {
        ...
        void Banner(SimEvent ev)
        {
            switch (ev.Kind)
            {
                ...
                case SimEventKind.Build:
                {
                    ...
                    if (planted.Type == BuildingType.Farm && !_farmPlanted)
                    {
                        _farmPlanted = true;
                    ...
                    if (planted.Type == BuildingType.Mine)
                    {
                        _padUntil = Time.time + 1.1f;
                        _padNodeId = planted.NodeId;
                        _padType = BuildingType.Mine;
                        _juice.PadDrop(ev.X, ev.Z, "MINE", new Color(0.94f, 0.64f, 0.23f));
                        break;
                    }
                    if (planted.Type == BuildingType.Power)
                    {
                        _padUntil = Time.time + 1.1f;
                        _padNodeId = planted.NodeId;
                        _padType = BuildingType.Power;
        ...
        void SyncView(bool _)
        {
            ...
            foreach (var n in _game.Nodes.Values)
            {
                ...
                else if (PadLive() && n.Id == _padNodeId)
                    c = PadTint() * (1.08f + 0.14f * Mathf.Abs(Mathf.Sin(Time.time * 11f)));
                ...
                else if (_game.GunsBackLive() && _game.Buildings.ContainsKey(n.Id)
                         && (n.Kind == NodeKind.Choke || n.Kind == NodeKind.Tower))
                    c = Color.Lerp(new Color(0.62f, 0.52f, 0.4f), new Color(0.48f * pulse, 0.95f * pulse, 0.62f), 0.7f);
                ...
                else if (n.Kind == NodeKind.Spawn)
                {
                    ...
                    MesaView.SetLabel(mark, splashWest ? "SPLASH"
                        ...
                        : _game.GunsBackLive() && _game.Buildings.ContainsKey(n.Id) ? "BACK"
                        : ChokeLabel(n.Id, near, chokeHot));
                else if (n.Kind == NodeKind.Tower)
                    MesaView.SetLabel(mark, _game.GunsUpNodeId() == n.Id ? "UP"
                        : _game.GunsBackLive() ? "BACK" : "GUN");
                else if (n.Kind == NodeKind.Pad)
                {
                    if (cut != null && (cut.A == n.Id || cut.B == n.Id))
                        ...
                        MesaView.SetLabel(mark, farmGlow ? "1 FARM" : "PAD");
                             ...
                             && (_game.GunsHungry() || _game.GunsUpLive() || _game.GunsBackLive()))
            ...
            foreach (var b in _game.Buildings.Values)
            {
                ...
                if ((b.Type == BuildingType.Kinetic || b.Type == BuildingType.Splash) && _game.PowerBrownout)
                    ...
                    tint = Color.Lerp(tint, new Color(0.48f, 0.95f, 0.62f), 0.45f * pulse);
                ...
                if (PlantLive() && b.Type == BuildingType.Farm)
                    tint = Color.Lerp(tint, new Color(0.5f, 0.85f, 0.48f), 0.55f * pulse);
                if (PadLive() && b.NodeId == _padNodeId)
                    tint = Color.Lerp(tint, PadTint(), 0.55f * pulse);
                ...
                if (b.Type == BuildingType.Hub && _game.Phase == Phase.Won)
                    ...
                    tr.localScale = _buildingScale[b.Id] * (1f + 0.16f * Mathf.Abs(Mathf.Sin(Time.time * 11f)));
                else if (PadLive() && b.NodeId == _padNodeId)
                    tr.localScale = _buildingScale[b.Id] * (1f + 0.16f * Mathf.Abs(Mathf.Sin(Time.time * 11f)));
        ...
        void SyncRings()
        {
            ...
            if (_game.GunsBackLive() && !_game.GunsDry() && !_game.RailLiveLive()
                && _game.Nodes.TryGetValue("hub", out var backHub))
            {
                live.Add("guns-back");
        ...
        void PingGunsClick()
        {
            ...
            if (_gunsClickJuiced) return;
            _gunsClickJuiced = true;
            foreach (var b in _game.Buildings.Values)
            {
                if ((b.Type != BuildingType.Kinetic && b.Type != BuildingType.Splash) || b.BuildLeft > 0)
                    continue;
                if (!_game.Nodes.TryGetValue(b.NodeId, out var n)) continue;
                _juice.GunsClick(n.X, n.Z);
            }
        }
        ...
        void PingGunsBack()
        {
            var key = _game.GunsBackPingKey();
            if (key == null)
            {
                _gunsBackPinged = null;
                return;
            }
            if (_gunsBackPinged == key) return;
            _gunsBackPinged = key;
            if (!_game.Nodes.TryGetValue("hub", out var hub)) return;
            _juice.GunsBack(hub.X, hub.Z);
            foreach (var b in _game.Buildings.Values)
            {
                if ((b.Type != BuildingType.Kinetic && b.Type != BuildingType.Splash) || b.BuildLeft > 0)
                    continue;
                if (!_game.Nodes.TryGetValue(b.NodeId, out var n)) continue;
                _juice.GunsFire(n.X, n.Z);
            }
            _hud.Flash(_game.GunsBackFlash() ?? "GUNS BACK — keep Power rolling", 2.0f,
        ...
        void SyncGunLocks()
        {
            ...
            foreach (var b in _game.Buildings.Values)
            {
                ...
                if (_game.PowerBrownout)
                    ...
                    color = new Color(0.48f, 0.95f, 0.62f, 0.9f);
        ...
        void DrawWorldBars()
        {
            ...
            if (_game.Nodes.TryGetValue("hub", out var hub))
            {
                ...
                if (hubSp.z > 0f)
                {
                    ...
                    if (_game.GunCountLive())
                    {
                        Color gunColor;
                        if (_game.GunsDry()) gunColor = new Color(0.72f, 0.18f, 0.1f, 0.92f);
                        else if (_game.GunsHungry()) gunColor = new Color(0.72f, 0.42f, 0.08f, 0.92f);
                        else gunColor = new Color(0.12f, 0.38f, 0.4f, 0.88f);
                        GUI.backgroundColor = gunColor;
                        GUI.Box(new Rect(hx + 44f, hy + 12f, 56f, 16f),
                    ...
                    if (_game.YardCountLive())
                    {
                        GUI.backgroundColor = _game.SitCountLive()
                            ? new Color(0.72f, 0.42f, 0.08f, 0.92f)
                            : new Color(0.28f, 0.32f, 0.28f, 0.88f);
                        GUI.Box(new Rect(hx - 40f, hy + 44f, 80f, 16f),
                    ...
                    if (_game.OffCountLive())
                    {
                        ...
                        GUI.Box(new Rect(hx - 102f, hy - 36f, 56f, 16f),
                            _game.OffCountChip() ?? "OFF");
                        GUI.backgroundColor = Color.white;
                    }
                    ...
                    if (_game.RaidPendingLive())
                    {
                        Color pendingColor;
                        if (_game.CoreThin)
                            pendingColor = new Color(0.72f, 0.42f, 0.08f, 0.92f);
                        else
                            pendingColor = new Color(0.48f, 0.18f, 0.14f, 0.88f);
                        GUI.backgroundColor = pendingColor;
                        GUI.Box(new Rect(hx + 44f, hy - 36f, 56f, 16f),
                            _game.RaidPendingChip() ?? "IN");
                        GUI.backgroundColor = Color.white;
                    }
                    if (_game.WaveDownLive())
                    {
                        Color downColor;
                        if (_game.WaveClearLive())
                            downColor = new Color(0.16f, 0.42f, 0.18f, 0.92f);
                        else if (_game.CoreThin)
                            downColor = new Color(0.72f, 0.42f, 0.08f, 0.92f);
                        else
                            downColor = new Color(0.42f, 0.22f, 0.12f, 0.88f);
                        GUI.backgroundColor = downColor;
                        GUI.Box(new Rect(hx - 40f, hy - 52f, 80f, 16f),
