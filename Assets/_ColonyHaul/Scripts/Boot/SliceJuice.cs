...
namespace ColonyHaul
{
    public sealed class SliceJuice
    {
        ...
        readonly AudioClip _nope;
        readonly AudioClip _stay;
        readonly AudioClip _free;
        readonly AudioClip _stow;
        ...
        public SliceJuice(Transform root, Camera cam)
        {
            ...
            _nope = Beep(160f, 0.12f);
            _stay = Beep(250f, 0.14f);
            _free = Beep(300f, 0.12f);
            _stow = Beep(220f, 0.1f);
        ...
        public void Nope(float x, float z, string tag)
        {
            Color color;
            switch (tag)
            {
                case "NEED":
                    ...
                    break;
                case "LOCK":
                    color = new Color(0.78f, 0.68f, 0.42f);
                    break;
                default:
                    color = new Color(1f, 0.42f, 0.28f);
                    break;
            }
            SpawnPip(x, z, string.IsNullOrEmpty(tag) ? "NOPE" : tag, color, 0.85f);
            SpawnBurst(x, z, new Color(color.r, color.g, color.b, 0.42f), 2.6f, 0.32f);
            Punch(0.18f);
            _audio.PlayOneShot(_nope, 0.48f);
        }

        public void Stay(float x, float z, bool farmFirst)
        {
            var color = farmFirst ? new Color(0.5f, 0.85f, 0.48f) : new Color(0.42f, 0.92f, 0.88f);
            SpawnPip(x, z, "STAY", color, 0.95f);
            SpawnBurst(x, z, new Color(color.r, color.g, color.b, 0.42f), 2.8f, 0.34f);
            Punch(0.2f);
            _audio.PlayOneShot(_stay, 0.5f);
        }

        public void Free(float x, float z)
        {
            var color = new Color(0.62f, 0.78f, 0.76f);
            SpawnPip(x, z, "FREE", color, 0.9f);
            SpawnBurst(x, z, new Color(0.62f, 0.78f, 0.76f, 0.4f), 2.6f, 0.32f);
            Punch(0.16f);
            _audio.PlayOneShot(_free, 0.48f);
        }

        public void Stow(float x, float z)
        {
            var color = new Color(0.72f, 0.74f, 0.7f);
            SpawnPip(x, z, "STOW", color, 0.85f);
            SpawnBurst(x, z, new Color(0.72f, 0.74f, 0.7f, 0.38f), 2.4f, 0.3f);
            Punch(0.14f);
            _audio.PlayOneShot(_stow, 0.42f);
        }
