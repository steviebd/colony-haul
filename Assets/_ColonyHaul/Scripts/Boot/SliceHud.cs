...
namespace ColonyHaul
{
    public sealed class SliceHud
    {
        ...
        void DrawTop(GameSim game)
        {
            ...
            else if (game.CoreThinLive())
                subCopy = game.CoreThinTitle();
        ...
        static void DrawLogistics(GameSim game)
        {
            ...
            else if (game.GunsBackLive()) guns = "GUNS BACK — " + game.GunsBackCall();
        ...
        void DrawTray(GameSim game)
        {
            ...
            if (game.HoldReady)
            {
                var holdPulse = game.HoldReadyLive() || (watch != null && !string.IsNullOrEmpty(watch.Copy) &&
                ...
                {
                    case HoldOrder.Auto:
                        holdLabel = game.HoldReadyLive() ? "▶ H Hold — GUNS / CREW" : "H Hold — auto";
